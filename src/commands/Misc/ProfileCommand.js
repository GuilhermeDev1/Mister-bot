...
module.exports = class Profile extends Command {
  ...
  async run(message, args) {
    const jeth = this.client.guilds.cache.get('1047342555685212261')
    ...
    let list = [];
    const dono_id = message.guild.ownerId;
    const dono = message.guild.members.cache.get(dono_id);
    const member = message.guild.members.cache.get(USER.id)
    if (process.env.OWNERS?.includes(message.author.id)) list.push('CREATOR');
    if (dono) list.push('SERVER_OWNER');
    if (jeth.members.cache.get(pUser.user.id)?.roles?.cache?.has('1047342688086806568')) list.push('STAFF');
    if (jeth.members.cache.get(pUser.user.id)?.roles?.cache?.has('1047342688086806568')) list.push('TRUST_SAFETY');
    if (jeth.members.cache.get(pUser.user.id)?.roles?.cache?.has('1047342688086806568')) list.push('JETH_PARTNER');
    if (jeth.members.cache.get(pUser.user.id)?.roles?.cache?.has('1047342688086806568')) list.push('DEV');
    if (jeth.members.cache.get(pUser.user.id)?.roles?.cache?.has('1047342688086806568')) list.push('BUG_HUNTER');
    if (member && member.premiumSinceTimestamp) list.push('BOOSTER');
    if (user.vip === true) list.push('VIP');
    if (user.isMarried) list.push('CASADO');
    if (jeth.members.cache.get(pUser.user.id)?.roles?.cache?.has('1047342688086806568')) list.push('PATREON');
